...
**项目结构：**
- `courseware/` - 课程材料（29个模块），包含课件PDF和参考代码
- `practice/` - 实践项目和案例研究（26个子项目）
- `practice-py/` - Python基础练习册
- `notebook/` - 学习笔记目录（与课程模块对应）
- `public/` - 公共资源（图片等）
- `docs/` - 项目文档
- `openspec/` - OpenSpec 变更管理配置
- `.iflow/` - iFlow CLI配置和技能（9个Skill）
...
├── notebook/                # 学习笔记（29个模块对应目录）
├── docs/                    # 项目文档
│   ├── iflow/               # iFlow 相关文档
│   │   ├── IFlow 配置区别.md
│   │   ├── iflow-agents-skills汇总.md
│   │   └── iFlow模型特性与使用建议.md
│   ├── homebrew/            # Homebrew 相关文档
│   ├── npm/                 # NPM 相关文档
│   └── AI主义历史.md
├── public/                  # 公共资源（图片等）
...
### 核心语言
- **Python**: >=3.11,<3.14（虚拟环境：3.11.14，类型检查：3.11）

### 包管理
- **uv**: 现代Python包管理器 v0.10.6（替代pip/venv）
- **统一依赖管理**: 所有子项目共享根目录虚拟环境
- **依赖查找顺序**:
  1. 当前启用的虚拟环境
  2. 当前目录的 `.venv` 目录
  3. uv 自己安装的 Python 环境
  4. 系统环境设定的 Python 环境
...
- **numpy**: >=2.4.2 - 数值计算
- **pandas**: >=2.0.0 - 数据分析
- **pyarrow**: >=23.0.1 - Apache Arrow 数据格式
...
- **modelscope**: >=1.34.0 - ModelScope模型库
...
项目配置了 9 个技能，用于扩展 iFlow CLI 的功能：

### 文档处理技能
- **docx**: Word文档创建、编辑和分析
- **pdf**: PDF操作、文本提取和表单处理
- **pptx**: PowerPoint演示文稿创建和编辑
- **xlsx**: Excel电子表格创建、编辑和数据分析

### 协作技能
- **doc-coauthoring**: 结构化文档协同创作工作流

### OpenSpec 技能
- **openspec-explore**: 探索模式 - 需求分析和问题探索
- **openspec-propose**: 提案模式 - 快速生成完整提案
- **openspec-apply-change**: 实施模式 - 执行变更任务
- **openspec-archive-change**: 归档模式 - 归档已完成的变更
...
项目配置了多个专业 Agent，详见 `docs/iflow-agents-skills汇总.md`。
...
项目使用 OpenSpec 进行结构化的变更管理：

1. **探索阶段** (`/opsx-explore`): 分析需求，探索问题空间
2. **提案阶段** (`/opsx-propose`): 生成包含设计、规范和任务的完整提案
3. **实施阶段** (`/opsx-apply`): 按任务执行变更
4. **归档阶段** (`/opsx-archive`): 归档已完成的变更
...
- [Python3教程](https://www.runnoob.com/python3/python3-tutorial.html)
- [大模型RAG基础](https://arthurchiao.art/blog/rag-basis-bge-zh/)
- [NLP教程](https://www.runnoob.com/nlp/nlp-tutorial.html)
...
*最后更新: 2026年3月3日*
*版本: 4.4*
*项目根目录 Python 版本: 3.11.14*
*项目要求 Python 版本: >=3.11,<3.14*
*类型检查 Python 版本: 3.11*