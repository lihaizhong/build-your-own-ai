...
from es_retrieval_tool import ElasticsearchRetrievalTool # 导入我们自定义的工具
...
你总是用中文回复用户。'''
    ...
    # --- 自定义工具集成 ---
    # 1. 实例化我们自定义的 ES 检索工具
    # 注意：这里的密码等信息是硬编码的，生产环境中建议通过配置文件或环境变量传入
    es_retrieval = ElasticsearchRetrievalTool(cfg={
        'password': 'euqPcOlHrmW18rtaS-3P' 
    })

    # 2. 定义智能体要使用的工具列表
    # 将 es_retrieval 实例放在列表前面，确保它能覆盖框架默认的 retrieval 工具
    tools = [es_retrieval, 'my_image_gen', 'code_interpreter'] 
