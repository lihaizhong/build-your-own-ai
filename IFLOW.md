...
- **最新版本**: V29版本持续优化中 (MAE = 542.3983)
- **目标突破**: 成功实现MAE < 500的阶段性目标
- **竞赛排名**: 多个版本在阿里云天池竞赛中表现优异
- **版本迭代**: 从v1的1000+分持续优化，V24-V28稳定在500分以内
- **技术积累**: 29个版本的完整迭代记录，包含大量优化经验
...
### 🛠️ 微调实操案例 (courseware/24-Fine-tuning实操)

深度模型微调的实战案例集合，包含完整的生产级应用：

**项目特点**:
- **李飞飞50美金复刻R1模型**: 基于s1: Simple test-time scaling方法
- **Unsloth高效微调**: 2-5倍速度提升，50-80%内存减少
- **Qwen2.5-7B微调**: Alpaca-cleaned数据集微调实战
- **中文医疗模型**: 垂类领域微调案例，涵盖6个医疗科室
- **GRPO强化学习**: 训练自己的推理模型，Group Relative Policy Optimization
- **Qwen-VL视觉微调**: 多模态模型微调，车辆里程表识别等应用
- **多数据集支持**: GSM8K、中文医疗数据、alpaca-cleaned等

**核心技术栈**:
- Unsloth框架: 高效微调，支持7GB显存训练15B参数模型
- TRL SFTTrainer: Hugging Face监督微调训练器
- GRPO算法: DeepSeek-R1核心技术
- LoRA/QLoRA: 参数高效微调方法
- 多模态微调: 支持文本+图像的联合微调

**运行方式**:
```bash
cd courseware/24-Fine-tuning实操
python Qwen2_5_(7B)_Alpaca-2.py     # Alpaca数据微调
python Qwen2_5_(7B)_医疗微调.py      # 医疗垂类模型
python Qwen2_5_(7B)_R1.py          # R1推理模型训练
python qwen_vl_car_insurance_train.py  # 视觉多模态微调
```

### 🏢 企业知识库实战 (courseware/25-项目实战：企业知识库)

企业级RAG系统实战开发，基于RAG-Challenge冠军方案：

**项目特点**:
- **RAG冠军方案**: 多路由+动态知识库，企业RAG挑战赛获奖方案
- **高质量解析**: Docling PDF解析，表格序列化，结构化提取
- **智能检索**: 混合搜索(向量+BM25)，父页面检索，LLM重排序
- **增强生成**: 思维链推理，结构化输出，指令细化
- **系统优化**: 超参数调优，性能监控，实时部署支持
- **中文适配**: 支持中文企业文档，DashScope API集成

**核心技术栈**:
- **解析引擎**: Docling、MinerU文档解析
- **向量数据库**: FAISS索引，混合检索策略
- **重排序模型**: Jina Reranker、LLM重排序
- **提示工程**: 结构化输出，多类型查询路由
- **部署方案**: Streamlit界面，API服务化
...
```bash
# 运行Unsloth微调
python Qwen2_5_(7B)_Alpaca-2.py

# 训练医疗垂类模型
python Qwen2_5_(7B)_医疗微调.py

# GRPO强化学习训练
python Qwen2_5_(7B)_R1.py
...
- 应用Unsloth框架实现2-5倍训练加速
- 使用GRPO算法进行推理能力训练
- 实施多模态联合微调策略

### 6. 企业级RAG系统优化
- 采用Docling或MinerU进行高质量文档解析
- 实现混合检索策略（向量+关键词）
- 应用LLM重排序提升检索精度
- 使用结构化输出确保答案格式一致性
- 实施父页面检索增强上下文完整性
...
- **RAG应用**: 构建了多个生产级RAG应用案例，从基础到企业级全覆盖
