...
from model.modeling_v11 import create_balanced_features, v11_optimize_model, get_project_path, get_user_data_path, load_and_preprocess_data
...
def load_predictions():
    """加载模型预测结果"""
    # 加载最新的V11模型预测结果
    pattern = get_project_path('prediction_result', 'modeling_v11_result_20251021_*.csv')
...
def main():
    ...
    # 创建保存目录
    # 使用绝对路径确保文件保存在正确位置
    save_path = get_project_path('user_data', 'modeling_v11')
    ...
    if predictions is None:
        ...
        # 训练模型并生成预测结果
        test_predictions = v11_optimize_model()
