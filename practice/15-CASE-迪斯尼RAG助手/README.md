# 迪斯尼RAG问答助手

基于RAG（Retrieval-Augmented Generation）技术的原生应用，支持文档处理、图像检索和混合问答。

## 项目简介

本项目实现了一个完整的RAG问答系统，通过FAISS向量数据库检索相关文档和图像，结合大语言模型生成准确的答案。系统支持迪斯尼相关的知识库问答，具备文档处理、图像OCR、混合检索等高级功能。

## 功能特性

### Step1: 数据层
- 📄 **文档处理**: 支持Word文档(.docx)、文本文件(.txt, .md)的解析
- 🖼️ **图像处理**: 支持图片OCR（Tesseract）和图像提取
- 📊 **表格提取**: 将Word文档中的表格转换为Markdown格式

### Step2: 向量化层
- 🎯 **文本Embedding**: 使用阿里云百炼text-embedding-v4模型（1024维）
- 🌟 **图像Embedding**: 使用CLIP模型提取图像特征（512维）
- 🗄️ **双索引系统**: FAISS分别构建文本和图像的向量索引

### Step3: 检索层
- 🔍 **混合检索**: 文本查询使用语义相似度，图像查询使用CLIP编码器
- ⚡ **关键词触发**: 检测特定关键词（如"海报"、"图片"）触发图像检索
- 📈 **相似度计算**: L2距离转相似度，支持分数阈值过滤

### Step4: 生成层
- 🤖 **上下文组织**: 将检索结果组织成结构化提示
- 💬 **智能问答**: 基于检索结果生成准确的答案
- 🔄 **流式输出**: 支持流式生成答案

## 项目结构

```
15-CASE-迪斯尼RAG助手/
├── code/                       # 核心代码
│   ├── __init__.py            # 模块初始化
│   ├── config.py              # 配置管理
│   ├── utils.py               # 工具函数
│   ├── data_processor.py      # 数据处理层（Step1）
│   ├── embedding.py           # 向量化层（Step2）
│   ├── retrieval.py           # 检索层（Step3）
│   ├── generator.py           # 生成层（Step4）
│   └── main.py                # 主程序入口
├── data/                       # 数据目录
│   └── images/                # 图像数据
├── user_data/                  # 用户数据目录
│   ├── documents/             # 文档数据
│   ├── indexes/               # FAISS索引文件
│   └── cache/                 # 缓存目录
├── output/                     # 输出目录
│   └── logs/                  # 日志文件
├── tests/                      # 测试目录
├── README.md                   # 项目说明
└── docs/                       # 文档目录
    └── USAGE.md                # 使用指南
```

## 环境要求

- Python >= 3.11
- uv 包管理器
- 根目录的虚拟环境

## 快速开始

### 1. 激活虚拟环境

```bash
# 激活根目录的虚拟环境
cd /Users/lihaizhong/Documents/Project/build-your-own-x/build-your-own-ai
source .venv/bin/activate
```

### 2. 安装依赖

确保根目录的 `pyproject.toml` 包含必要的依赖：

```toml
dependencies = [
    "langchain>=0.1.0",
    "langchain-community>=0.1.0",
    "langchain-core>=0.1.0",
    "langchain-openai>=0.1.0",
    "faiss-cpu>=1.7.4",
    "PyPDF2>=3.0.0",
    "python-docx>=0.8.11",
    "pytesseract>=0.3.10",
    "python-dotenv>=1.0.0",
    "dashscope>=1.22.1",
    "numpy>=1.24.0",
    "pandas>=2.0.0",
    "pillow>=10.0.0",
    "torch>=2.0.0",
    "transformers>=4.30.0",
    "openai>=1.0.0",
    "loguru>=0.7.0",
]
```

如果缺少依赖，运行：

```bash
uv add langchain langchain-community langchain-core langchain-openai
uv add faiss-cpu PyPDF2 python-docx pytesseract
uv add python-dotenv dashscope numpy pandas
uv add pillow torch transformers openai loguru
```

### 3. 配置环境变量

确保根目录的 `.env` 文件包含：

```env
DASHSCOPE_API_KEY=your_dashscope_api_key_here
```

### 4. 准备数据

项目已包含示例文档：
- `user_data/documents/disney_classics.md` - 迪士尼经典动画电影
- `user_data/documents/disney_parks.md` - 迪士尼乐园介绍

你可以添加自己的文档和图像：

```bash
# 文档放入 user_data/documents/
cp your_document.docx user_data/documents/

# 图像放入 data/images/
cp your_image.png data/images/
```

### 5. 构建索引

```bash
cd practice/15-CASE-迪斯尼RAG助手
python -m code.main --build
```

### 6. 运行问答

#### 交互模式

```bash
python -m code.main --interactive
```

#### 单次查询

```bash
python -m code.main --query "迪士尼有哪些经典动画电影？"
```

## 技术栈

- **LangChain**: 大模型应用开发框架
- **FAISS**: 向量相似度搜索
- **DashScope**: 阿里云通义千问API（文本Embedding和LLM）
- **OpenAI**: 兼容API客户端
- **Transformers**: CLIP模型（图像Embedding）
- **Tesseract**: OCR文本识别
- **PyTorch**: 深度学习框架

## 模块说明

### 数据处理层 (`data_processor.py`)

- `DocumentProcessor`: 文档处理器，支持Word和文本文件
- `ImageProcessor`: 图像处理器，支持OCR识别
- `TextChunk`: 文本块数据类
- `ImageData`: 图像数据类

### 向量化层 (`embedding.py`)

- `TextEmbeddingModel`: 文本向量化（阿里云百炼）
- `ImageEmbeddingModel`: 图像向量化（CLIP）
- `FAISSIndex`: FAISS索引管理
- `VectorStore`: 向量存储管理器

### 检索层 (`retrieval.py`)

- `TextRetriever`: 文本检索器
- `ImageRetriever`: 图像检索器
- `HybridRetriever`: 混合检索器
- `RetrievalResult`: 检索结果数据类

### 生成层 (`generator.py`)

- `PromptBuilder`: 提示词构建器
- `AnswerGenerator`: 答案生成器
- `RAGPipeline`: RAG流程管道

### 主程序 (`main.py`)

提供命令行界面：
- `--build`: 构建索引
- `--interactive`: 交互式问答
- `--query`: 单次查询

## 使用示例

### 示例1: 文本问答

```bash
$ python -m code.main -q "迪士尼有哪些经典动画电影？"

问题: 迪士尼有哪些经典动画电影？

答案:
迪士尼有许多经典动画电影，包括：

1. 米老鼠系列 - 1928年创造的经典角色
2. 《白雪公主与七个小矮人》(1937) - 迪士尼第一部动画长片
3. 《木偶奇遇记》(1940)
4. 《灰姑娘》(1950)
5. 《睡美人》(1959)
6. 《小美人鱼》(1989)
7. 《狮子王》(1994)
```

### 示例2: 图像触发检索

```bash
$ python -m code.main -q "展示一下迪士尼的海报"

问题: 展示一下迪士尼的海报

答案:
迪士尼有众多经典电影的海报...

相关图像信息：
- 图像1 (来源: data/images/lion_king_poster.jpg, 相似度: 0.856)
- 图像2 (来源: data/images/snow_white_poster.jpg, 相似度: 0.832)
```

## 配置说明

主要配置在 `code/config.py` 中：

```python
@dataclass
class Config:
    # Embedding配置
    text_embedding_model: str = "text-embedding-v4"
    text_embedding_dim: int = 1024
    image_embedding_dim: int = 512
    clip_model_name: str = "ViT-B/32"
    
    # 检索配置
    top_k: int = 5
    score_threshold: float = 0.7
    
    # 图像检索关键词
    image_keywords: list = ["海报", "图片", "图像", "照片", "截图", "展示"]
```

## 详细使用指南

更多详细使用说明请查看 [USAGE.md](./docs/USAGE.md)。

## 常见问题

### Q1: 如何添加新文档？

将文档文件放入 `user_data/documents/` 目录，然后重新构建索引。

### Q2: 索引文件保存在哪里？

索引文件保存在 `user_data/indexes/` 目录。

### Q3: 支持哪些文档格式？

支持 `.docx`、`.txt`、`.md` 格式。

### Q4: 支持哪些图像格式？

支持 `.png`、`.jpg`、`.jpeg`、`.gif`、`.bmp`、`.webp` 格式。

## 许可证

本项目遵循项目根目录下的LICENSE文件中定义的许可证。

## 更新日志

### v1.0.0 (2026-02-10)
- ✨ 完整的RAG流程实现
- 📄 支持Word文档处理
- 🖼️ 支持图像OCR和向量化
- 🔍 混合检索和关键词触发
- 💬 交互式问答界面
- 📚 完整的使用文档