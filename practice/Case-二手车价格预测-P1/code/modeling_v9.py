...
def train_models_with_cv(X_train, y_train, X_test):
    ...
    lgb_params = {
        ...
        'num_leaves': 8,  # 极简复杂度
        'max_depth': 2,    # 极简深度
        'min_data_in_leaf': 500,  # 增加最小样本数
        'feature_fraction': 0.4,  # 降低特征采样
        'bagging_fraction': 0.4,  # 降低数据采样
        'bagging_freq': 5,
        'lambda_l1': 5.0,  # 强正则化
        'lambda_l2': 5.0,  # 强正则化
        'min_gain_to_split': 0.5,  # 增加分裂最小增益
        'verbose': -1,
        'random_state': 42
    }
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X_train), 1):
        ...
        # 训练LightGBM
        lgb_model = lgb.LGBMRegressor(**lgb_params, n_estimators=50) # type: ignore
        ...
        # 训练XGBoost
        xgb_model = xgb.XGBRegressor(**xgb_params, n_estimators=50)
...
def advanced_ensemble(lgb_pred, xgb_pred, cat_pred, y_train):
    ...
    # XGBoost表现中等，给予中等权重
    # CatBoost表现较差，给予较低权重
    ensemble_pred = (0.7 * lgb_pred + 0.2 * xgb_pred + 0.1 * cat_pred)
...
def v9_optimize_model():
    ...
    result_file = os.path.join(result_dir, f"modeling_v9_result_{timestamp}.csv")
...
if __name__ == "__main__":
    test_pred = v9_optimize_model()
    print("V9优化完成!请将生成的CSV文件提交到平台验证效果。")