...
def augment_data(train_df, y_train, n_augment=2):
    """
    数据增强技术
    通过添加噪声来增加训练数据的多样性
    """
    ...
    for i in range(n_augment):
        ...
        for col in numeric_cols:
            noise = np.random.normal(0, 0.01 * augmented_df[col].std(), size=len(augmented_df))
            augmented_df[col] = augmented_df[col] + noise
        
        # 为目标变量添加小幅度噪声
        y_noise = np.random.normal(0, 0.01 * augmented_y.std(), size=len(augmented_y))
...
def train_models_with_cv(X_train, y_train, X_test):
    ...
    lgb_params = {
        ...
        'num_leaves': 6,  # 极简复杂度
        'max_depth': 2,    # 极简深度
        'min_data_in_leaf': 800,  # 增加最小样本数
        'feature_fraction': 0.3,  # 降低特征采样
        'bagging_fraction': 0.3,  # 降低数据采样
        'bagging_freq': 5,
        'lambda_l1': 8.0,  # 强正则化
        'lambda_l2': 8.0,  # 强正则化
        'min_gain_to_split': 1.0,  # 增加分裂最小增益
        'verbose': -1,
        'random_state': 42
    }
    ...
    xgb_params = {
        ...
        'reg_alpha': 8.0,
        'reg_lambda': 8.0,
        'min_child_weight': 100,  # 增加子节点最小权重
        'random_state': 42
    }
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X_train), 1):
        ...
        # 训练LightGBM
        lgb_model = lgb.LGBMRegressor(**lgb_params, n_estimators=30) # type: ignore
        ...
        # 训练XGBoost
        xgb_model = xgb.XGBRegressor(**xgb_params, n_estimators=30)
...
def advanced_ensemble(lgb_pred, xgb_pred, cat_pred, y_train):
    ...
    # XGBoost表现中等，给予中等权重
    # CatBoost表现较差，给予较低权重
    ensemble_pred = (0.8 * lgb_pred + 0.15 * xgb_pred + 0.05 * cat_pred)
...
def v10_optimize_model():
    ...
    train_df = create_optimized_features(train_df)
    test_df = create_optimized_features(test_df)
    ...
    result_file = os.path.join(result_dir, f"modeling_v10_result_{timestamp}.csv")
    ...
    # 保存模型
    if models_to_save:
        save_models(models_to_save, 'v10')
...
if __name__ == "__main__":
    test_pred = v10_optimize_model()
    print("V10优化完成!请将生成的CSV文件提交到平台验证效果。")