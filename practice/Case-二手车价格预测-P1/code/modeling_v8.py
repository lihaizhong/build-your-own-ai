...
def train_models_with_cv(X_train, y_train, X_test):
    ...
    lgb_params = {
        ...
        'max_depth': 3,    # 进一步降低深度
        'min_data_in_leaf': 300,  # 增加最小样本数
        'feature_fraction': 0.5,  # 降低特征采样
        'bagging_fraction': 0.5,  # 降低数据采样
        'bagging_freq': 5,
        'lambda_l1': 2.0,  # 增加L1正则化
        'lambda_l2': 2.0,  # 增加L2正则化
        'min_gain_to_split': 0.2,  # 增加分裂最小增益
        'verbose': -1,
        'random_state': 42
    }
    ...
    catboost_params = {
        ...
        'depth': 3,
        'learning_rate': 0.02,  # 进一步降低学习率
        'iterations': 80,  # 减少迭代次数
        'l2_leaf_reg': 2.0,
        'random_strength': 1.0,  # 增加随机强度
        'random_seed': 42,
        'verbose': False
    }
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X_train), 1):
        ...
        # 训练LightGBM
        lgb_model = lgb.LGBMRegressor(**lgb_params, n_estimators=80) # type: ignore
        ...
        # 训练XGBoost
        xgb_model = xgb.XGBRegressor(**xgb_params, n_estimators=80)
...
def advanced_ensemble(lgb_pred, xgb_pred, cat_pred, y_train):
    ...
    # XGBoost表现中等，给予中等权重
    # CatBoost表现较差，给予较低权重
    ensemble_pred = (0.6 * lgb_pred + 0.25 * xgb_pred + 0.15 * cat_pred)
...
def v8_optimize_model():
    ...
    result_file = os.path.join(result_dir, f"modeling_v8_result_{timestamp}.csv")
...
if __name__ == "__main__":
    test_pred = v8_optimize_model()
    print("V8优化完成!请将生成的CSV文件提交到平台验证效果。")