"""
V27完整版本模型 - 精准突破450分完整优化

基于V27核心版本的534.93分基础，实施完整优化策略:
1. 多模型集成 - LightGBM + XGBoost + CatBoost
2. 增强特征工程 - 更多高价值特征，目标编码
3. 高级校准方法 - 分位数校准+分布调整
4. 智能权重调整 - 基于验证分数和稳定性的动态权重
5. 超参数微调 - 基于核心版本的参数进一步优化

目标：MAE < 450，精准突破450分大关
"""
...
def enhanced_preprocessing():
    ...
    if 'power' in all_df.columns:
        ...
        all_df['power_is_medium'] = ((all_df['power'] > 100) & (all_df['power'] <= 200)).astype(int)
        ...
        all_df['power_squared'] = np.power(all_df['power'], 2) / 1000
    ...
    for col in categorical_cols:
        if col in all_df.columns:
            ...
            # 排序编码
            label_map = {v: i for i, v in enumerate(sorted(all_df[col].unique()))}
            all_df[f'{col}_rank'] = all_df[col].map(label_map)
    ...
    # 周特征（模拟）
    all_df['reg_weekday'] = np.random.randint(0, 7, size=len(all_df))
    all_df['reg_is_weekend'] = (all_df['reg_weekday'] >= 5).astype(int)
    ...
    if 'price' in all_df.columns:
        ...
        brand_stats['smooth_std'] = ((brand_stats['std'] * np.sqrt(brand_stats['count']) + 
                                    global_std * np.sqrt(smooth_factor)) / np.sqrt(brand_stats['count'] + smooth_factor))
        
        # 映射特征
        brand_mean_map = brand_stats.set_index('brand')['smooth_mean']
        brand_std_map = brand_stats.set_index('brand')['smooth_std']
        ...
        all_df['brand_std_price'] = all_df['brand'].map(brand_std_map).fillna(global_std)
...
def create_enhanced_features(df):
    ...
    if 'power' in df.columns and 'car_age' in df.columns:
        ...
        df['power_age_product'] = df['power'] * df['car_age']
    ...
    if 'kilometer' in df.columns and 'car_age' in df.columns:
        ...
        df['km_age_diff'] = df['kilometer'] - df['car_age'] * 15000
    ...
    for col in ['car_age', 'kilometer', 'power']:
        if col in df.columns:
            ...
            if col != 'power':  # power的平方已经在预处理中处理了
                df[f'{col}_squared'] = np.power(df[col], 2) / 1000
    ...
    if len(v_cols) >= 5:
        ...
        if v_positive:
            ...
            df['v_pos_sum'] = df[v_positive].sum(axis=1)
        if v_negative:
            df['v_neg_mean'] = df[v_negative].mean(axis=1)
            df['v_neg_sum'] = df[v_negative].sum(axis=1)
    ...
    if 'fuelType' in df.columns and 'gearbox' in df.columns:
        df['fuel_gearbox'] = df['fuelType'].astype(str) + '_' + df['gearbox'].astype(str)
        le = LabelEncoder()
        df['fuel_gearbox'] = le.fit_transform(df['fuel_gearbox'])
...
def train_complete_ensemble(X_train, y_train, X_test):
    """
    训练完整集成模型 - 多模型集成，智能权重
    """
    print("训练完整集成模型...")
    ...
    lgb_params = {
        'objective': 'mae',
        'metric': 'mae',
        'num_leaves': 43,        # 基于核心版本增加
        'max_depth': 9,          # 基于核心版本增加
        'learning_rate': 0.055,  # 基于核心版本微调
        'feature_fraction': 0.85, # 基于核心版本增加
        'bagging_fraction': 0.85,
        'bagging_freq': 5,
        'lambda_l1': 0.2,        # 基于核心版本降低
        'lambda_l2': 0.2,        # 基于核心版本降低
        'min_child_samples': 18, # 基于核心版本降低
        'random_state': 42,
    }
    ...
    xgb_params = {
        ...
        'eval_metric': 'mae',
        'max_depth': 9,          # 与LightGBM保持一致
        'learning_rate': 0.055,  # 与LightGBM保持一致
        'subsample': 0.85,       # 与LightGBM保持一致
        'colsample_bytree': 0.85,
        'colsample_bylevel': 0.85,
        'reg_alpha': 0.6,        # 适度正则化
        'reg_lambda': 0.6,       # 适度正则化
        'min_child_weight': 8,   # 适度正则化
        'random_state': 42
    }
    ...
    catboost_params = {
        ...
        'eval_metric': 'MAE',
        'depth': 9,              # 与其他模型保持一致
        'learning_rate': 0.055,  # 与其他模型保持一致
        'iterations': 1500,      # 平衡速度和性能
        'l2_leaf_reg': 2.5,      # 适度正则化
        'random_strength': 0.3,  # 适度随机性
...
def advanced_calibration(predictions, y_train):
    ...
    if pred_mean > 0:
        ...
        mean_calibration = np.clip(mean_calibration, 0.85, 1.15)
    else:
    ...
    if calib_std > 0:
        ...
        std_calibration = np.clip(std_calibration, 0.85, 1.15)
    else:
        std_calibration = 1.0
    ...
    try:
        ...
        pred_quantiles = np.percentile(calibrated_predictions, [5, 10, 25, 50, 75, 90, 95])
        
        # 分段映射函数
        def quantile_mapping(x):
            if x <= pred_quantiles[0]:
                return train_quantiles[0]
            elif x <= pred_quantiles[1]:
                ratio = (x - pred_quantiles[0]) / (pred_quantiles[1] - pred_quantiles[0])
                return train_quantiles[0] + ratio * (train_quantiles[1] - train_quantiles[0])
            elif x <= pred_quantiles[2]:
                ratio = (x - pred_quantiles[1]) / (pred_quantiles[2] - pred_quantiles[1])
                return train_quantiles[1] + ratio * (train_quantiles[2] - train_quantiles[1])
            elif x <= pred_quantiles[3]:
                ratio = (x - pred_quantiles[2]) / (pred_quantiles[3] - pred_quantiles[2])
                return train_quantiles[2] + ratio * (train_quantiles[3] - train_quantiles[2])
            elif x <= pred_quantiles[4]:
                ratio = (x - pred_quantiles[3]) / (pred_quantiles[4] - pred_quantiles[3])
                return train_quantiles[3] + ratio * (train_quantiles[4] - train_quantiles[3])
            elif x <= pred_quantiles[5]:
                ratio = (x - pred_quantiles[4]) / (pred_quantiles[5] - pred_quantiles[4])
                return train_quantiles[4] + ratio * (train_quantiles[5] - train_quantiles[4])
            elif x <= pred_quantiles[6]:
                ratio = (x - pred_quantiles[5]) / (pred_quantiles[6] - pred_quantiles[5])
                return train_quantiles[5] + ratio * (train_quantiles[6] - train_quantiles[5])
            else:
                return train_quantiles[6]
        
        # 应用分位数校准
        quantile_calibrated = np.array([quantile_mapping(x) for x in calibrated_predictions])
        
        # 混合原始预测和分位数校准结果
        calibrated_predictions = 0.7 * calibrated_predictions + 0.3 * quantile_calibrated
...
def v27_complete():
    ...
    print("=" * 80)
    print("开始V27完整版本模型训练")
    print("基于V27核心版本的534.93分基础，实施完整优化策略")
    print("目标：MAE < 450，精准突破450分大关")
    print("=" * 80)
    ...
    top_features = correlations.head(60).index.tolist()
    ...
    # 步骤5: 训练完整集成模型
    print("\n步骤5: 训练完整集成模型...")
    ensemble_pred, scores_info = train_complete_ensemble(X_train, y_train, X_test)
    ...
    result_file = os.path.join(result_dir, f"modeling_v27_{timestamp}.csv")
    submission_df.to_csv(result_file, index=False)
    print(f"\nV27完整版本结果已保存到: {result_file}")
    ...
    # 保存模型
    if models_to_save:
        save_models(models_to_save, 'v27')
    ...
    print("=" * 80)
    print("✅ 基于V27核心版本的534.93分基础，实施完整优化策略")
    print("✅ 多模型集成 - LightGBM + XGBoost + CatBoost")
    print("✅ 增强特征工程 - 更多高价值特征，目标编码")
    print("✅ 高级校准方法 - 分位数校准+分布调整")
    print("✅ 智能权重调整 - 基于验证分数和稳定性的动态权重")
    print("✅ 超参数微调 - 基于核心版本的参数进一步优化")
    print("🚀 目标：精准突破450分大关!")
    print("=" * 80)
...
if __name__ == "__main__":
    test_pred, scores_info = v27_complete()
    print("V27完整版本完成! 期待突破450分! 🚀")
