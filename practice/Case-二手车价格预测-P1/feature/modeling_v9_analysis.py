...
from model.modeling_v9 import create_minimal_features, v9_optimize_model, get_project_path, get_user_data_path, load_and_preprocess_data
...
def load_predictions():
    """加载模型预测结果"""
    # 加载最新的V9模型预测结果
    pattern = get_project_path('prediction_result', 'modeling_v9_result_20251021_*.csv')
...
def plot_learning_curve(train_df, save_path):
    ...
    # 随机采样10%的数据
    sample_size = min(10000, len(X_train))  # 最多使用10000个样本
    ...
    learning_curve_result = learning_curve(
        ...
        cv=3,  # 减少交叉验证折数以提高速度
        n_jobs=-1,
        train_sizes=np.linspace(0.1, 1.0, 6),  # 减少训练尺寸点数
...
def main():
    ...
    # 创建保存目录
    # 使用绝对路径确保文件保存在正确位置
    save_path = get_project_path('user_data', 'modeling_v9')
    ...
    if predictions is None:
        ...
        # 训练模型并生成预测结果
        test_predictions = v9_optimize_model()
