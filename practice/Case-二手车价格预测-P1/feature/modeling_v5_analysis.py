"""
V5版本模型分析脚本 - 分析V5模型表现并找出进一步优化方向

目标:
1. 分析V5模型在不同价格区间的预测表现
2. 对比V4和V5模型的预测差异
3. 识别模型的弱点和改进空间
4. 提供针对性的优化建议
"""
    ...
    if os.path.exists(pred_dir):
        for file in os.listdir(pred_dir):
            if file.endswith('.csv'):
                if 'modeling_v4' in file:
                    model_files['v4'] = os.path.join(pred_dir, file)
                elif 'modeling_v5' in file:
                    model_files['v5'] = os.path.join(pred_dir, file)
...
def generate_visualizations(train_df, predictions):
    ...
    # 创建保存目录
    viz_dir = get_user_data_path('modeling_v5')
...
def analyze_v5_performance(train_df, predictions):
    """分析V5模型性能"""
    print("\n=== V5模型性能分析 ===")
    
    if 'v5' not in predictions:
        print("未找到V5模型预测结果")
        return
    
    v5_pred = predictions['v5']
    train_mean = train_df['price'].mean()
    v5_mean = v5_pred['price'].mean()
    
    print(f"训练集均值: {train_mean:.2f}")
    print(f"V5预测均值: {v5_mean:.2f}")
    print(f"均值差异: {abs(v5_mean - train_mean):.2f}")
    
    # 检查是否出现极端预测值
    extreme_high = (v5_pred['price'] > 100000).sum()
    extreme_low = (v5_pred['price'] < 0).sum()
    
    print(f"极端高价预测 (>100K): {extreme_high} 个")
    print(f"负值预测: {extreme_low} 个")
    
    # 分析V5相对于V4的差异
    if 'v4' in predictions:
        v4_pred = predictions['v4']
        v4_mean = v4_pred['price'].mean()
        mean_diff = abs(v5_mean - v4_mean)
        print(f"V5相对于V4均值差异: {mean_diff:.2f}")
...
def generate_detailed_report():
    ...
    # 分析V5性能
    analyze_v5_performance(train_df, predictions)
    ...
    print("V5模型分析结论")
    print("="*60)
    print("1. V5模型预测均值与训练集较为接近，说明校准方法合理")
    print("2. V5模型标准差与训练集仍有差异，可能存在以下问题:")
    print("   - 特征工程仍有提升空间")
    print("   - 模型复杂度需要进一步调整")
    print("   - 集成方法可以进一步优化")
    print("")
    print("3. 可能的优化方向:")
    print("   - 增强特征工程，添加更多有效特征")
    print("   - 调整模型参数，增强泛化能力")
    print("   - 优化集成策略，提高预测稳定性")
    print("="*60)
