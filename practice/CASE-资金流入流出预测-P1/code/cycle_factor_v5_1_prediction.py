...
纯周期因子预测模型 v5.1
基于v5版本的精细调优版本
版本特性：精准校准节假日效应 + 渐进式月末效应 + 稳定性约束
核心创新：解决v5版本的关键问题，目标冲击120+分
演进：v5版本 + 精确参数调优
"""
...
def calculate_smart_enhanced_trend_v5_1(data, target_col, outlier_threshold=0.95):
    """计算v5.1智能增强趋势（优化多时间窗口权重）"""
    print("=== 计算v5.1智能增强多时间窗口移动平均趋势 ===")
    ...
    # 3. v5.1优化权重组合（降低短期权重，提升稳定性）
    print("3. 优化权重组合...")
    
    # 动态权重：根据数据稳定性调整权重（v5.1优化）
    recent_data = data[target_col].tail(30)
    volatility = recent_data.std() / recent_data.mean()
    
    if volatility < 0.15:  # 低波动期
        weights = [0.05, 0.20, 0.35, 0.40]  # 提升长期权重，降低短期
    elif volatility < 0.25:  # 中等波动期（与v5相同）
        weights = [0.10, 0.30, 0.30, 0.30]  # 平衡权重
    else:  # 高波动期
    ...
    # 5. 最终增强趋势：v5.1更保守的增强
    enhancement_factor = 1 + trend_change_rate * 0.2  # 从30%降至20%，更保守
    smart_enhanced_trend = smart_trend * enhancement_factor
    
    print(f"v5.1智能趋势计算完成:")
...
def analyze_historical_holiday_effects(data):
    ...
    if holiday_effects:
        ...
        print(f"  v5.1使用更保守的节假日效应")
        
        # v5.1使用更保守的节假日效应（最大-8%）
        return min(avg_purchase_effect, -0.08)  # 最大-8%影响
    else:
...
def calculate_enhanced_weekday_factors(data, trend_data):
    """计算v5.1增强的weekday周期因子"""
    print("=== 计算v5.1增强Weekday周期因子 ===")
...
def calculate_enhanced_day_factors(data, trend_data, purchase_weekday_factors, redeem_weekday_factors):
    ...
    print("v5.1增强Day因子计算完成（显示部分主要日期）:")
...
def calculate_smart_trend_prediction_v5_1(data, trend_data, future_dates):
    """v5.1版本智能趋势预测（增强稳定性）"""
    print("=== 计算v5.1智能趋势预测 ===")
    ...
    max_change_rate = 0.10  # v5.1新增：限制最大变化率10%
    ...
    for i, date in enumerate(future_dates):
        ...
        # v5.1新增：稳定性约束
        if i > 0:
            prev_trend = purchase_trend_pred[-1]
            change_rate = abs(trend_base - prev_trend) / max(prev_trend, 1)
            
            # 限制变化率
            if change_rate > max_change_rate:
                # 按最大变化率限制
                if trend_base > prev_trend:
                    trend_base = prev_trend * (1 + max_change_rate)
                else:
                    trend_base = prev_trend * (1 - max_change_rate)
    ...
    print(f"v5.1智能趋势预测完成，9月1日趋势: 申购¥{purchase_trend_pred[0]:,.0f}, 赎回¥{redeem_trend_pred[0]:,.0f}")
    return purchase_trend_pred, redeem_trend_pred


def apply_refined_business_logic_v5_1(predictions, holiday_effect):
    """应用v5.1精细化业务逻辑（精准校准版）"""
    print("=== 应用v5.1精细化业务逻辑 ===")
    ...
    mid_autumn_effects = {
        6: -0.08,  # 9月6日最明显负效应（-8%）
        7: -0.06,  # 9月7日适中负效应（-6%）
        8: -0.04   # 9月8日轻微负效应（-4%），确保三天持续体现
    }
    
    for pred in predictions:
        # 中秋节三天精确效应
        if pred['day'] in [6, 7, 8]:
            effect = mid_autumn_effects[pred['day']]
            pred['purchase_pred'] *= (1 + effect)  # 精确负效应
            pred['redeem_pred'] *= (1 + effect * 0.8)  # 赎回影响稍小
            pred['business_logic_type'] = f'中秋节效应({effect*100:.0f}%)'
        elif pred['day'] in [28]:  # 9月28日（调休上班日）
            pred['purchase_pred'] *= 1.05  # 调休上班日申购增加（从8%降至5%）
            pred['redeem_pred'] *= 1.03    # 赎回轻微增加（从5%降至3%）
            pred['business_logic_type'] = '调休上班效应'
    
    # 2. v5.1优化渐进式季度末效应
    print("2. 优化渐进式季度末效应处理...")
    for pred in predictions:
        if pred['day'] >= 25:  # 月末效应
            days_from_25 = pred['day'] - 25
            # v5.1优化：更渐进的变化率（从2%降至0.8%）
            month_end_factor = 1 + 0.008 * days_from_25  # 每天递增0.8%
            
            pred['purchase_pred'] *= month_end_factor
            pred['redeem_pred'] *= (1 + 0.006 * days_from_25)  # 赎回效应更小
            pred['business_logic_type'] = '月末渐进效应'
            
            if pred['day'] == 30:  # 9月30日特殊处理（v5.1优化）
                pred['purchase_pred'] *= 1.02  # Q3季度末特殊加成（从3%降至2%）
                pred['business_logic_type'] = '季度末结算效应'
    
    # 3. v5.1简化双周模式（专注于关键效应）
    print("3. 简化双周模式建模...")
    for pred in predictions:
        if pred['week_of_month'] == 5:  # 最后一个周末
            if pred['weekday'] >= 5:  # 周六周日
                pred['purchase_pred'] *= 0.97  # 最后一个周末效应更明显（从3%降至3%）
    ...
    for i, pred in enumerate(predictions):
        if i > 0:
            ...
            # 限制相邻天的最大变化
            purchase_change = abs(pred['purchase_pred'] - prev_pred['purchase_pred']) / max(prev_pred['purchase_pred'], 1)
            redeem_change = abs(pred['redeem_pred'] - prev_pred['redeem_pred']) / max(prev_pred['redeem_pred'], 1)
            
            # 如果变化过大，进行平滑处理
            max_daily_change = 0.15  # 限制最大日变化15%
            
            if purchase_change > max_daily_change:
                if pred['purchase_pred'] > prev_pred['purchase_pred']:
                    pred['purchase_pred'] = prev_pred['purchase_pred'] * (1 + max_daily_change)
                else:
                    pred['purchase_pred'] = prev_pred['purchase_pred'] * (1 - max_daily_change)
            
            if redeem_change > max_daily_change:
                if pred['redeem_pred'] > prev_pred['redeem_pred']:
                    pred['redeem_pred'] = prev_pred['redeem_pred'] * (1 + max_daily_change)
                else:
                    pred['redeem_pred'] = prev_pred['redeem_pred'] * (1 - max_daily_change)
    
    print("v5.1精细化业务逻辑应用完成")
    return predictions


def predict_september_2014_v5_1(data, purchase_trend_pred, redeem_trend_pred, 
                               purchase_weekday_factors, redeem_weekday_factors,
                               purchase_day_factors, redeem_day_factors, holiday_effect):
    """v5.1版本预测2014年9月的申购赎回金额"""
    print("=== v5.1版本预测2014年9月 ===")
    ...
    # 应用v5.1精细化业务逻辑
    predictions = apply_refined_business_logic_v5_1(predictions, holiday_effect)
    
    return predictions


def calculate_confidence_scores_v5_1(predictions, data, trend_data):
    """计算v5.1版本的置信度"""
    print("=== 计算v5.1版本置信度 ===")
    ...
    if len(mid_autumn_days) > 0:
        ...
        if avg_normal_purchase > 0:
            ...
            if 0.04 <= holiday_effect_ratio <= 0.10:  # 节假日效应在4-10%之间
    ...
    if len(end_of_month) > 0 and len(mid_month) > 0:
        ...
        if avg_mid_purchase > 0:
            ...
            if 0.01 <= month_end_effect <= 0.12:  # 月末效应在1-12%之间
    ...
    if purchase_cv < 0.3 and redeem_cv < 0.3:
        stability_score = 20
    elif purchase_cv < 0.4 and redeem_cv < 0.4:
        stability_score = 15
    else:
        stability_score = 10
    ...
    if avg_change_rate < 0.12:  # 平均变化率小于12%（从15%优化至12%）
        quality_score += 10
...
def save_predictions_v5_1(predictions):
    """保存v5.1预测结果"""
    print("=== 保存v5.1预测结果 ===")
    ...
    # 保存为CSV（考试格式）
    prediction_file = get_project_path('..', 'prediction_result', 'cycle_factor_v5_1_predictions_201409.csv')
    ...
    # 保存详细结果
    detailed_file = get_project_path('..', 'user_data', 'cycle_factor_v5_1_detailed_201409.csv')
...
def print_prediction_summary_v5_1(predictions):
    """打印v5.1预测摘要"""
    print("\n" + "="*80)
    print("🎯 v5.1精细调优版周期因子预测摘要")
    print("="*80)
    ...
    print(f"🎯 v5.1版本置信度: {avg_confidence:.1f}")
    
    print(f"\n📊 v5.1版本精细调优亮点:")
    print(f"  ✅ 精准节假日效应: 9月6-8日(-8%, -6%, -4%)")
    print(f"  ✅ 渐进式月末效应: 每天0.8%递增，30日+2%特殊加成")
    print(f"  ✅ 优化时间窗口权重: 降低短期权重，提升稳定性")
    print(f"  ✅ 稳定性约束: 限制最大变化率15%，平均变化率<12%")
    print(f"  ✅ 预测连续性: 智能平滑处理相邻天变化")
    
    # 关键日期对比分析（v3 vs v5.1）
    print(f"\n📊 关键日期对比 (v3 vs v5.1 vs v5):")
    
    # 9月1日
    v3_day1 = 325636082
    v5_day1 = 335979787
    v5_1_day1 = predictions[0]['purchase_pred']
    print(f"  9月1日: v3{v3_day1:,.0f} vs v5.1{v5_1_day1:,.0f} vs v5{v5_day1:,.0f}")
    
    # 中秋节三天
    mid_autumn_days = [p for p in predictions if p['day'] in [6, 7, 8]]
    v3_mid_autumn = {6: 164939912, 7: 179234400, 8: 343805692}
    v5_mid_autumn = {6: 161602351, 7: 180608897, 8: 377281321}
    
    for day in mid_autumn_days:
        day_num = day['day']
        v5_1_val = day['purchase_pred']
        v3_val = v3_mid_autumn[day_num]
        v5_val = v5_mid_autumn[day_num]
        effect_type = day.get('business_logic_type', '未知')
        
        print(f"  9月{day_num}日: v3{v3_val:,.0f} vs v5.1{v5_1_val:,.0f} vs v5{v5_val:,.0f} ({effect_type})")
    
    # 月末三天
    end_month_days = [p for p in predictions if p['day'] in [28, 29, 30]]
    v3_end_month = {28: 207840488, 29: 333163885, 30: 276619558}
    v5_end_month = {28: 454398838, 29: 675378352, 30: 627467527}
    
    for day in end_month_days:
        day_num = day['day']
        v5_1_val = day['purchase_pred']
        v3_val = v3_end_month[day_num]
        v5_val = v5_end_month[day_num]
        effect_type = day.get('business_logic_type', '未知')
    ...
    print(f"\n📊 v5.1改进效果:")
    print(f"  关键问题修复:")
    print(f"    - 9月8日: v5 ¥377M → v5.1 ¥{mid_autumn_days[2]['purchase_pred']:,.0f} ({effect_type})")
    print(f"    - 9月30日: v5 ¥627M → v5.1 ¥{end_month_days[2]['purchase_pred']:,.0f} ({end_month_days[2].get('business_logic_type', '未知')})")


def main():
    """主函数"""
    print("=== v5.1精细调优版周期因子预测分析 ===")
    print("🎯 基于v5版本 + 精准校准，目标冲击120+分")
    print("📊 v5.1优化版：精准节假日效应 + 渐进式月末效应 + 稳定性约束")
    ...
    try:
        ...
        # 3. 计算智能增强趋势（v5.1优化）
        purchase_trend_data = calculate_smart_enhanced_trend_v5_1(data, 'purchase')
        redeem_trend_data = calculate_smart_enhanced_trend_v5_1(data, 'redeem')
        ...
        # 6. 计算v5.1智能趋势预测
        future_dates = pd.date_range(start='2014-09-01', end='2014-09-30', freq='D')
        purchase_trend_pred, redeem_trend_pred = calculate_smart_trend_prediction_v5_1(
            data, purchase_trend_data, future_dates)
        
        # 7. 生成v5.1预测
        predictions = predict_september_2014_v5_1(
            data, purchase_trend_pred, redeem_trend_pred,
            purchase_weekday_factors, redeem_weekday_factors,
            purchase_day_factors, redeem_day_factors, holiday_effect)
        
        # 8. 计算v5.1置信度
        predictions = calculate_confidence_scores_v5_1(predictions, data, purchase_trend_data)
        
        # 9. 保存结果
        prediction_file, detailed_file = save_predictions_v5_1(predictions)
        
        # 10. 打印摘要
        print_prediction_summary_v5_1(predictions)
        
        print(f"\n=== v5.1精细调优预测完成 ===")
        print(f"✅ v5.1精细调优模型预测成功")
        print(f"📊 预测结果已保存")
        print(f"📈 可查看文件:")
