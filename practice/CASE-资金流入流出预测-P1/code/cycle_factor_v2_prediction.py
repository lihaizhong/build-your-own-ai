...
def calculate_confidence_scores_v2(predictions, data, purchase_trend, redeem_trend, 
                          purchase_weekday_factors, redeem_weekday_factors,
                          purchase_day_factors, redeem_day_factors):
    """计算整体方案的置信度分数（0-100）- 改进版"""
    print("=== 计算整体方案置信度 ===")
    ...
    base_consistency_score = 0
    if 0.2 <= purchase_cv <= 3.0 and 0.2 <= redeem_cv <= 3.0:
        base_consistency_score = 5  # 基础一致性分数
    elif 0.1 <= purchase_cv <= 4.0 and 0.1 <= redeem_cv <= 4.0:
        base_consistency_score = 3  # 基本可接受
    else:
        base_consistency_score = 1  # 较差
    
    # 4.2 趋势连续性检查（新增）
    trend_consistency_score = 0
    purchase_diffs = [purchase_preds[i+1] - purchase_preds[i] for i in range(len(purchase_preds)-1)]
    redeem_diffs = [redeem_preds[i+1] - redeem_preds[i] for i in range(len(redeem_preds)-1)]
    
    # 计算相邻天变化率的合理性
    purchase_change_rates = [abs(d) / max(purchase_preds[i], 1) for i, d in enumerate(purchase_diffs)]
    redeem_change_rates = [abs(d) / max(redeem_preds[i], 1) for i, d in enumerate(redeem_diffs)]
    
    # 大变化（>50%）应该是少数
    large_changes_purchase = sum(1 for rate in purchase_change_rates if rate > 0.5)
    large_changes_redeem = sum(1 for rate in redeem_change_rates if rate > 0.5)
    
    if large_changes_purchase <= 3 and large_changes_redeem <= 3:  # 最多3个大变化
        trend_consistency_score = 5
    elif large_changes_purchase <= 5 and large_changes_redeem <= 5:
        trend_consistency_score = 3
    else:
    ...
    # 检查周末vs工作日的合理性
    weekend_purchase_avg = np.mean([purchase_preds[i] for i, p in enumerate(predictions) if p['weekday'] >= 5])
    workday_purchase_avg = np.mean([purchase_preds[i] for i, p in enumerate(predictions) if p['weekday'] < 5])
    
    weekend_redeem_avg = np.mean([redeem_preds[i] for i, p in enumerate(predictions) if p['weekday'] >= 5])
    workday_redeem_avg = np.mean([redeem_preds[i] for i, p in enumerate(predictions) if p['weekday'] < 5])
    
    # 周末应该比工作日低（符合历史规律）
    if weekend_purchase_avg < workday_purchase_avg and weekend_redeem_avg < workday_redeem_avg:
        business_logic_score += 3
    
    # 检查月初月末的合理性（简单检查）
    early_month_purchase_avg = np.mean([purchase_preds[i] for i, p in enumerate(predictions) if p['day'] <= 10])
    late_month_purchase_avg = np.mean([purchase_preds[i] for i, p in enumerate(predictions) if p['day'] >= 21])
    
    if abs(early_month_purchase_avg - late_month_purchase_avg) / max(early_month_purchase_avg, 1) < 0.5:
        business_logic_score += 2
    
    # 综合预测一致性分数
    prediction_consistency_score = base_consistency_score + trend_consistency_score + business_logic_score
    prediction_consistency_score = min(prediction_consistency_score, 15)  # 最高15分
    ...
    print(f"    - 基础一致性: {base_consistency_score}/5")
    print(f"    - 趋势连续性: {trend_consistency_score}/5")
    print(f"    - 业务逻辑: {business_logic_score}/5")
    if len(test_data) >= 30:
...
def save_predictions(predictions):
    ...
    # 保存为CSV（考试格式）
    prediction_file = get_project_path('..', 'prediction_result', 'cycle_factor_v2_predictions_201409.csv')
    ...
    # 保存详细结果
    detailed_file = get_project_path('..', 'user_data', 'cycle_factor_v2_detailed_201409.csv')
...
def main():
    """主函数"""
    print("=== 纯周期因子资金流入流出预测分析 ===")
    print("🎯 基于weekday和day周期因子的时间序列分解预测")
    print("📊 优化版：改进预测一致性和置信度评估体系")
    ...
    try:
        ...
        # 7. 计算置信度（改进版）
        predictions = calculate_confidence_scores_v2(predictions, data, purchase_trend, redeem_trend,
                                                    purchase_weekday_factors, redeem_weekday_factors,
                                                    purchase_day_factors, redeem_day_factors)
