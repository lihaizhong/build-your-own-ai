...
class DifyAgentClient:
  ...
  def chat_completion(
    ...
    Args:
      ...
      conversation_id (str, optional): 会话 ID，用于维持对话上下文
    ...
    """
    if app_type == "auto":
      # 先尝试 chat 端点，如果失败则尝试 completion 端点，最后尝试 workflow 端点
  ...
  def _try_chat_endpoint(
    ...
    # 只有当 conversation_id 不为 None 且不为空字符串时才添加
    if conversation_id:
  ...
  def run_workflow(
    ...
    Args:
      inputs (dict): 工作流输入参数
      user_id (str): 用户 ID，默认为 "default_user"
      stream (bool): 是否使用流式响应，默认为 False
    """
    url = f"{self.base_url}/workflows/run"
  ...
  def completion_message(
    ...
    """
    直接调用 completion 端点，支持自定义 inputs
    ...
    for input_format in possible_inputs:
      ...
      try:
        ...
        # 如果是 app_unavailable 错误，尝试下一种格式
  ...
  def _handle_workflow_streaming_response(
    self,
    url: str,
    payload: Dict[str, Any]
  ) -> Dict[str, Any]:
