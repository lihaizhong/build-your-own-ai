...
from model.modeling_v6 import create_advanced_features, v6_optimize_model, get_project_path, get_user_data_path, load_and_preprocess_data
...
def load_predictions():
    ...
    # 加载最新的V6模型预测结果
    pred_paths = [
        get_project_path('prediction_result', 'modeling_v6_result_20251021_111136.csv'),
        get_project_path('prediction_result', 'modeling_v6_result_20251021_111007.csv'),
        get_project_path('prediction_result', 'modeling_v6_result_20251021_110156.csv')
    ]
    
    for pred_path in pred_paths:
        if os.path.exists(pred_path):
            predictions = pd.read_csv(pred_path)
            print(f"成功加载预测文件: {pred_path}")
            return predictions
...
def main():
    ...
    # 创建保存目录
    # 使用绝对路径确保文件保存在正确位置
    save_path = get_project_path('user_data', 'modeling_v6')
    ...
    if predictions is None:
        ...
        # 训练模型并生成预测结果
        test_predictions = v6_optimize_model()
        ...
        result_file = os.path.join(result_dir, f"modeling_v6_result_{timestamp}.csv")
